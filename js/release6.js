            ...
            success:function (d) {
                if(d.status == 'success'){
                    ...
                    S.each(d.picture, function (item) {
                        p+="<a class='J_img' href='#' onclick='aa(\""+item+"\")'><img width=50 height=50 src=\"" + item + "\" /></a>";
                    });
...
function aa(s){
    ...
    */
    ...
    if(inArray(value, photo_arr)){
        var index = inArray(value, photo_arr);
        photo_arr.splice(index, 1);
        $('#photo2').val(photo_arr.join(' '));
    }
    else{    
        photo_arr.push(value);
        $('#photo2').val(photo_arr.join(' '));
    }
}
